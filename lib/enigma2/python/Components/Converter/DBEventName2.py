...
class DBEventName2(Converter, object):
	NAME = 0
	SHORT_DESCRIPTION = 1
	...
	def getText(self):
		event = self.source.event
		if event is None:
			return pgettext("No event available", "No event available")

		if self.type is self.NAME:
			if event.getEventName() == "Current":
				return pgettext("Current", "Current")
			elif event.getEventName() == "Next":
				return pgettext("Next", "Next")
			elif event.getEventName() == "No Schedule":
				return pgettext("No Schedule", "No Schedule")
			elif event.getEventName() == "dummyEventName":
				return pgettext("dummyEventName", "dummyEventName")
			else:
			...
			if event.getExtendedDescription() or event.getShortDescription() == "Not Applicable":
				return pgettext("Not Applicable", "Not Applicable")
			else:
			...
			if description:
				if extended:
					...
					return pgettext("Event description unavailable", "Event description unavailable")
				elif extended == "  ":
					return pgettext("Event description unavailable", "Event description unavailable")
				elif description == "No Schedule":
					return pgettext("No Schedule", "No Schedule")
				elif extended == "No Schedule":
					return pgettext("No Schedule", "No Schedule")
				elif description == "Not Applicable":
					return pgettext("Not Applicable", "Not Applicable")
				elif extended == "Not Applicable":
					return pgettext("Not Applicable", "Not Applicable")
				elif description == "dummyEventDescription":
					return pgettext("dummyEventDescription", "dummyEventDescription")
				elif extended == "dummyEventDescription":
					return pgettext("dummyEventDescription", "dummyEventDescription")
